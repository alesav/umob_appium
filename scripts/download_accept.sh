...
TOTAL_BUILD_COUNT=$(curl -s -u :$TOKEN "$BASE_URL/_apis/build/builds?definitions=10&api-version=7.1-preview.7" | \
  jq -r '.count')
echo "Total number of BUILD_IDS available: $TOTAL_BUILD_COUNT"

# Get 50 latest build IDs
echo "Fetching 50 latest build IDs..."
BUILD_IDS=$(curl -s -u :$TOKEN "$BASE_URL/_apis/build/builds?definitions=10&api-version=7.1-preview.7" | \
  jq -r '.value | sort_by(.startTime) | reverse | .[0:50] | map(.id) | .[]')

# Process each build ID until we find one with an android-Accept artifact
for BUILD_ID in $BUILD_IDS; do
  echo "Checking build $BUILD_ID for android-Accept artifact..."

  # Try to get the download URL for android-Accept artifact
  DOWNLOAD_URL=$(curl -s -u :$TOKEN "$BASE_URL/_apis/build/builds/$BUILD_ID/artifacts?api-version=7.1-preview.5" | \
    jq -r '.value[] | select(.name=="android-Accept") | .resource.downloadUrl')

  # If download URL exists (not empty), download the artifact
  if [ -n "$DOWNLOAD_URL" ]; then
    echo "Found android-Accept artifact in build $BUILD_ID. Downloading..."

    ZIP_FILE="$TEMP_DIR/android-Accept-build-$BUILD_ID.zip"
    ...
    echo "✗ No android-Accept artifact found in build $BUILD_ID"
  fi

  echo "------------------------"
done

echo "No android-Accept artifacts were found in the latest 50 builds."
