...
class FelyxScooterActions {
  ...
  static async clickStartTripButton() {
    const startTripButton = await driver.$('-android uiautomator:new UiSelector().text("START TRIP")');
...
describe('Felyx Scooter Booking - New User Without Card', () => {
  ...
  it('New User Tries to Reserve Felyx Scooter Without Card', async () => {
    await TestRunner.runTest("f8c39b91-153c-431c-8c49-8bf1246f7416", async () => {
      // Find a Felyx scooter from the fetched coordinates
      const targetScooter = scooters.find(scooter => scooter.id.includes('Felyx'));
      
      if (!targetScooter) {
        throw new Error('No Felyx scooter found in the fetched coordinates');
      }
