...
class WebView {
                ...
                return (
                    ...
                    currentContexts.find((context) => {
                        if (driver.isIOS) {
                            ...
                            return (
                                (context as IosContext).bundleId ===
                                    appIdentifier &&
                                (context as ContextInterface)?.url !==
                                    "about:blank"
                            );
                        }
                        ...
                        return (
                            (context as AndroidContext).packageName ===
                                appIdentifier &&
                            (context as AndroidContext)?.androidWebviewData
                                ?.empty === false
                        );
                    })
                );
    ...
    private findMatchingContext({
        ...
        return contexts.find((context) => {
            const idMatch = driver.isIOS
                ? (context as IosContext).bundleId === identifier
                : (context as AndroidContext).packageName === identifier;
            ...
            const additionalChecks = driver.isIOS
                ? true
                : (context as AndroidContext).androidWebviewData?.attached &&
                  (context as AndroidContext).androidWebviewData?.visible;
     ...
     * NOTE: This is an internal method and should not be called outside of this class
     */
    private generateNonMatchingErrorMessage({
        identifier,
        title,
        url,
    }: {
    ...
    async getCurrentContexts(
        ...
        const contexts = (await driver.execute("mobile: getContexts")) as
            | IosContexts
            | AndroidInternalContexts;
        ...
        const parsedAndroidContexts = await this.parsedAndroidContexts(
            contexts as AndroidInternalContexts,
            packageName,
        );
        ...
        const isAndroidWebviewDataMissing =
            androidContext && !("androidWebviewData" in androidContext);
        ...
        const isAndroidWebviewDataEmpty =
            androidContext && androidContext.androidWebviewData?.empty;

        if (
            isPackageNameMissing ||
            isAndroidWebviewDataMissing ||
            isAndroidWebviewDataEmpty
        ) {
            ...
            throw new Error(
                `The packageName '${packageName}' matches, but no webview with pages was loaded in this response: '${JSON.stringify(contexts)}'`,
            );
        }
     ...
     */
    async parsedAndroidContexts(
        contexts: AndroidInternalContexts,
        packageName: string,
    ): Promise<AndroidContexts> {
