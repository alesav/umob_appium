 ...
 */
export async function clickAccountAndWaitForPersonalInfo(
    driver,
    PageObjects,
    maxRetries = 5,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            console.log(
                `Attempt ${attempt}: Clicking account button and looking for Personal info`,
            );
            ...
            try {
                ...
                console.log(
                    `Success: Personal info found on attempt ${attempt}`,
                );
                return personalInfo;
            } catch (waitError) {
                console.log(
                    `Attempt ${attempt}: Personal info not found, will retry...`,
                );
                lastError = waitError;
    ...
    throw new Error(
        `Failed to find Personal info after ${maxRetries} attempts. Last error: ${lastError?.message}`,
    );
}
