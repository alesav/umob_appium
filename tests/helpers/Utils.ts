...
export function isIosRealDevice() {
    ...
    return (
        ...
        realDeviceRegex.test(driver.capabilities["appium:udid"] as string)
    );
}
...
export async function relaunchApp(identifier: string) {
    const appIdentifier = {
        [driver.isAndroid ? "appId" : "bundleId"]: identifier,
    };
 ...
 */
export async function executeInHomeScreenContext(
    action: () => Promise<void>,
): Promise<any> {
    ...
    const activeAppInfo: AppInfo = await driver.execute(
